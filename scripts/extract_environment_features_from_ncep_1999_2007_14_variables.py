...
def load_dataset(path: str) -> xr.Dataset:
    ...
    surface_ds = load_grib1(
        path, filter_by_keys=dict(typeOfLevel='surface'))
    print("before rename", surface_ds)     
    surface_ds = surface_ds.rename_vars(dict(t='tsfc'))
    print("after rename", surface_ds)
    

    # Load tropopause event for 2 new variables
    trop_2_ds = load_grib1(path, filter_by_keys = dict(typeOfLevel ="tropopause"))
    trop_2_ds = trop_2_ds.rename_vars(dict(t="temp", gh="htrp"))
    print("trop_2_ds", trop_2_ds)   
    

    # Merge datasets.
    merged_ds = common_ds.merge(surface_ds)

    merged_ds = merged_ds.merge(trop_2_ds)
    print("Merged ds before rename", merged_ds)
    rename_vars = dict(
        u='ugrdprs',
        v='vgrdprs',
        w='vvelprs',
    ...
    # Rename variables to what we usually do.
    merged_ds = merged_ds.rename_vars(rename_vars)
    print("Merged ds after ", merged_ds)
